...
#ifndef MC_DNA_ENCODING_HPP_
#define MC_DNA_ENCODING_HPP_


#include "bitmanip.hpp"
#include "sequence_iostream.hpp"
...
//-----------------------------------------------------------------------------
/**
 * @breif facilities for creating/manipulating 2bit encoding of DNA
 */
namespace detail {
    ...
    const numk_t k, InputIterator first, InputIterator last, Consumer&& consume)
{
    for_each_kmer_2bit<UInt>(k, first, last,
        [&] (UInt kmer, half_size_t<UInt> ambig) {
            if (not ambig) consume(make_canonical_2bit(kmer, k));
        });
}
