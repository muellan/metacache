...
#ifndef MC_CMDLINE_INTERFACE_HPP_
#define MC_CMDLINE_INTERFACE_HPP_
...
struct build_options
{
    ...
    std::size_t maxPartBytes = std::numeric_limits<std::size_t>::max();
#endif

    taxonomy_options taxonomy;
    bool resetParents = false;
    // process multiple database parts in parallel
...
{
    std::string dbfile;
    int dbpart = -1;  // default: all parts
    std::vector<std::string> infiles;
