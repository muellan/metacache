...
#ifndef MC_DATABASE_QUERY_HPP_
#define MC_DATABASE_QUERY_HPP_
 ...
 */
        ...
        std::mutex& scheduleMtx)
    {
        for (const auto& query : sequenceBatch) {
            ...
            if (not queryBatch.add_paired_read(hostId, query.seq1, query.seq2,
                                               opt.sketching, rules))
            {
    ...
    InfoCallback&& showInfo)
{
    query_database(infilenames, db, opt,
       ...
       [&] (std::exception& e) { 
            if (opt.output.showErrors) std::cerr << "FAIL: " << e.what() << '\n'; 
       }
    );
}
