...
#ifndef MC_SKETCH_DATABASE_HPP_
#define MC_SKETCH_DATABASE_HPP_
...
public:
    ...
    //---------------------------------------------------------------
    part_id part_count () const noexcept {
        return featureStore_.table_count();
    }


    //---------------------------------------------------------------
    void initialize_parts (part_id numParts) {
        featureStore_.initialize_tables(numParts);
    }

#ifdef GPU_MODE
    //---------------------------------------------------------------
    bool merge_reduce_max_parts_max_bytes
        (unsigned, std::size_t,
         int = 1, info_level = info_level::moderate) { return false; }
    ...
    //---------------------------------------------------------------
    void add_empty_part () {
        featureStore_.add_empty_table();
    }

    void remove_empty_parts () {
        featureStore_.remove_empty_tables();
    }

    bool merge_reduce_max_parts_max_bytes
        (unsigned maxCount, std::size_t maxBytes,
         int maxConcurrency = 1, info_level info = info_level::moderate)
    {
        if (maxBytes < 1) return false;
        if (maxCount < 1) return false;
        if (maxCount >  featureStore_.table_count()) return false;
        if (maxCount == featureStore_.table_count()) return true;

        return featureStore_.merge_reduce_max_tables_max_bytes(maxCount, maxBytes, maxConcurrency, info);
    }
#endif
    ...
    /** @brief clears feature map without memory deallocation */
    void clear_without_deallocation();


    //---------------------------------------------------------------
    const taxonomy_cache& taxa () const noexcept { return taxonomyCache_; }
          taxonomy_cache& taxa ()       noexcept { return taxonomyCache_; }


    //---------------------------------------------------------------
#ifdef GPU_MODE
    void query_gpu_async (
     ...
     */
    void write (const std::string& filename, info_level) const;
...
private:
     ...
     */
    void write_meta (const std::string& filename, info_level) const;
    void write_cache (const std::string& filename, part_id partId, info_level) const;
