...
/*************************************************************************//**
 *
 * @brief hasher that uses the all kmers for sketch
 *
 *****************************************************************************/
template<class KmerT, class Hash = same_size_hash<KmerT>>
class single_function_hasher
{
public:
    //---------------------------------------------------------------
    using kmer_type    = KmerT;
    ...
    //---------------------------------------------------------------
    explicit
    single_function_hasher(hasher hash = hasher{}):
        hash_(std::move(hash)), k_(16), sketchSize_(~0),
        windowSize_(127), windowStride_(127-k_+1)
    {}
                    ...
                    Consumer&& consume) const
    {
        for_each_window(first, last, windowSize_, windowStride_,
            [&] (InputIterator first, InputIterator last) {
                ...
                auto sketch = sketch_type{};
                sketch.reserve(s);

                for_each_unambiguous_canonical_kmer_2bit<kmer_type>(k_, first, last,
                    [&] (kmer_type kmer) {
                        auto h = hash_(kmer);
                        sketch.emplace_back(h);
                    });
    ...
    //---------------------------------------------------------------
    friend void
    write_binary(std::ostream& os, const single_function_hasher& h)
    {
        write_binary(os, std::uint64_t(h.k_));
        write_binary(os, std::uint64_t(h.sketchSize_));
        write_binary(os, std::uint64_t(h.windowSize_));
    ...
    //---------------------------------------------------------------
    friend void
    read_binary(std::istream& is, single_function_hasher& h)
    {
        std::uint64_t n = 0;
        read_binary(is, n);
        h.k_ = (n <= max_kmer_size()) ? n : max_kmer_size();
