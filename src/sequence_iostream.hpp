...
//-----------------------------------------------------------------------------
/**
 * @brief  buffered character sequence reader with ZLIB support
 */
class char_istream
{
public:
    //---------------------------------------------------------------
    enum class file_status : char {
        no_err   = 0, // no error
        eof      = 1, // end-of-file
    ...
    struct filehandle
    {
        ...
        file_status open (const char *filename) {
            if (file_) close();
            file_ = gzopen(filename, "r");
            if (file_ == Z_NULL) return file_status::err;
            return file_status::no_err;
        }
        void close () {
            gzclose(file_);
        }
        ...
        file_status open (const char *filename) {
            ...
            if (file_ == NULL) return file_status::err;
            return file_status::no_err;
        }
        void close () {
            fclose(file_);
...
private:
    ...
    //---------------------------------------------------------------
    static constexpr size_t bufsize () noexcept { return 1U << 16 ; }
...
public:
    ...
    char_istream () :
        ...
        begin_{0}, end_{0},
        lastChar_{0},
        status_{file_status::err}
    {}

    explicit
    char_istream (const char* filename) : char_istream {}
    {
        open(filename);
    }
    ...
    //---------------------------------------------------------------
    file_status status () const noexcept { return status_; }
    bool good () const noexcept { return status_ == file_status::no_err; }
...
private:
    ...
    //---------------------------------------------------------------
    bool do_buffered_read () {
        if (status_ != file_status::no_err) return false;
