...
#ifndef MC_SKETCHERS_HPP_
#define MC_SKETCHERS_HPP_
...
struct sketching_options
{
    ...
    //---------------------------------------------------------------
    friend
    void write_binary (std::ostream& os, const sketching_options<kmer_type>& h)
    {
    ...
    //---------------------------------------------------------------
    friend 
    void read_binary (std::istream& is, sketching_options<kmer_type>& h)
    {
        std::uint64_t n = 0;
        read_binary(is, n);
        ...
        Consumer&& consume)
    {
        ...
        for_each_window(first, last, opt.winlen, opt.winstride,
            [&] (InputIterator first, InputIterator last) {
                ...
                if (not sketch.empty() && sketch.back() == feature_type(~0)) {
