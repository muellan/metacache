...
#ifndef MC_SEQUENCE_VIEW_HPP_
#define MC_SEQUENCE_VIEW_HPP_
...
public:
    ...
    decltype(auto)
    operator [] (size_type i) const noexcept { return beg_[i]; }


    bool empty() const noexcept { return beg_ == end_; }
   
    explicit
    operator bool () const noexcept(noexcept(empty())) { return not empty(); }


    iterator begin () const noexcept { return beg_; }
